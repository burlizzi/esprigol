...
void FingerprintGrowComponent::enroll_fingerprint(uint16_t finger_id, uint8_t num_buffers) {
  ...
  uint8_t parameterBits = 0x00;
  //if (!requireFingerRemoval) parameterBits |= (1 << 5);
  //if (allowOverwrite) parameterBits |= (1 << 3);
  //if (!checkDuplicate) parameterBits |= (1 << 4);
  this->data_ = {AUTO_ENROLL, (uint8_t) (finger_id >> 8), (uint8_t) (finger_id & 0xFF),num_buffers, (uint8_t) (parameterBits >> 8), (uint8_t) (parameterBits & 0xFF)};
  delay(20);  // This delay guarantees the sensor will in fact be powered power.
  auto result = this->send_command_();
  switch (result) {
    case OK:
...
void FingerprintGrowComponent::aura_led_control(uint8_t state, uint8_t speed, uint8_t color, uint8_t count) {
  ...
  this->data_ = {AURA_CONFIG, state,color, (state==1?0:color), count};
  switch (this->send_command_()) {
    case OK:
